...
```
./install.sh --hooks                     # 每轮对话后刷新，不靠定时器
...
默认是定时器 10 分钟一跑。能用，但那是轮询：你空闲时它照样烧请求，你密集用时它又最多滞后 10 分钟。

`--hooks` 换成事件驱动 —— 由 Claude Code 的 `Stop` 和 `SessionStart` 事件触发，每轮对话结束几秒内就更新，而那正是配额变动的时刻。

```bash
./install.sh --hooks
```

**不会增加任何 API 请求**，因为每条来源有自己的最小间隔：

| 来源 | 最小间隔 | 理由 |
|---|---|---|
| cswap | 10s（仅防抖） | cswap **内部**就有 180s 的新鲜度地板，且**所有读它的调用方共享同一个地板**。调得再频繁也产生不了上游流量：要么免费读它的存储，要么触发一次它本来也要做的抓取。 |
| oauth | 300s | 每次都是真·1 个请求，而预算是每 token 每滚动小时约 28–30 个。这个预算不是漏桶 —— 一次突发会把它饱和整整一小时 —— 而且 cswap 已经占掉了大部分。 |
| claude-cli | 600s | 同样的成本，外加一个子进程和一轮 hooks。 |

某条来源被节流时**整个运行直接结束**，而不是往下掉到更贵的那条。被节流意味着快照已经和这次触发能拿到的一样新，再花一个请求去问同一个数字就本末倒置了。只有**真正失败**才会向下 fallback —— 这让节流顺带成了熔断器：便宜那条哪天开始静默失效，每轮触发也烧不穿预算。

**即使开了 hooks 也要留着定时器**（它会自动放慢到 30 分钟）。配额是按身份算的，你在 claude.ai 网页版、手机 App 或另一台机器上的消耗，永远不会触发本机的 `Stop` hook。

事件驱动下，成功但数值没变的运行**不写日志** —— 否则日志会被一模一样的行淹掉。失败永远写。`HUD_FEEDER_VERBOSE=1` 可以看到每一次决策，包括被节流的。
